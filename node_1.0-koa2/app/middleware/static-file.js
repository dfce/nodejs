...
function staticFiles ( url, dir ) {
    return async ( ctx, next ) => {
        ...
        if (fpath == '/MP_verify_tpviPnKEgGUyMjnP.txt') {
            let fp = path.dirname(__dirname) + '/MP_verify_tpviPnKEgGUyMjnP.txt'

            ctx.response.type = mime.getType( fp);// 查找文件的mime:
        ...
        if ( fpath.startsWith(url) ) {
            ...
            if ( await fs.exists( fp ) ) {
                ...
                ctx.response.type = mime.getType( fpath );   // 同上 mime 2.0 以上版本 lookup 函数名修改为 getType
